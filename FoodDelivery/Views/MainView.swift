...
struct MainView: View {
  ...
  @State private var size = UIScreen.main.bounds.width
  
  var body: some View {
    ZStack {
      VStack {
        HeaderView(title: coordinator.category?.name ?? "")
          .padding(.bottom, 8)
        tabView()
      }
      ...
      if catVM.showCard {
        ...
        DishCardView(dish: catVM.currentDish ?? Dish.testDish, showCard: $catVM.showCard)
  ...
  private func tabView() -> some View {
    TabView(selection: $coordinator.currentPage) {
      CoordinatorView()
        ...
        .tabItem { tabItemView(image: R.TabIcons.home, "Главная") }
      Text("Search")
        .tag(Page.search)
        .tabItem { tabItemView(image: R.TabIcons.search, "Поиск") }
      CartView()
        .environmentObject(cartVM)
        .tag(Page.cart)
        .badge(cartVM.positions.count)
        .tabItem { tabItemView(image: R.TabIcons.cart, "Корзина") }
      Text("Profile")
        .tag(Page.profile)
        .tabItem { tabItemView(image: R.TabIcons.profile, "Аккаунт") }
    }
    .accentColor(Color.blueBtn)
  }
  
  @ViewBuilder
  private func tabItemView(image: String, _ text: String) -> some View {
    VStack {
      Image(image)
